...
PROMPT='
%{$fg[yellow]%}%m%{$reset_color%} %{$fg[white]%}く%{$reset_color%} %{$fg[red]%}%(5~|%-1~/…/%3~|%4~) %{$reset_color%}$(git_prompt_short_sha)$(git_prompt_info)
$(prompt_char) '
...
RPROMPT='%{$fg[green]%}$(if [ $cmd_time ]; then echo "($cmd_time) "; fi)%'